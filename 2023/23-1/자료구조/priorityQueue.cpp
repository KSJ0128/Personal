...
class MaxHeap
{
    ...
    void insert(int key)
    {
        ...
        while(i != 1 && key > getParent(i).getKey()) // 루트 아니고, 부모 노드보다 키 값이 크면
        {
    ...
    HeapNode remove()
    {
        ...
        {
            if(child < size && 
                getLeft(parent).getKey() < getRight(parent).getKey()) // 동레벨에서 더 큰 자식 노드로 이동
            {
                child++;
            }
            
            if(last.getKey() >= node[child].getKey()) // 마지막 노드가 자식 노드보다 크면
            {
                break;
            }
    ...
    HeapNode find() // 우선 순위 가장 높은 요소 리턴
    {
        return node[1];
    }
...
void heapSortDec(int a[], int n) // 내림차순 정렬
{
    MaxHeap heap;
...
int main()
{
    int arr[10] = {0, 10, -3, 7, 22, 33, 39, 4, -9, -5};
