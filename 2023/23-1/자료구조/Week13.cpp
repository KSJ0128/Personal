...
int main() {
	// p. 466
	// 1) 가중치 그래프를 Adjacent Matrix 로 표현하기("graph.txt")를 이용하니 파일 내용을 확인하세요. 
	//WGraph WG;
	//string fn = "graph.txt";
	//WG.load(fn);
	//WG.display();
	...
	// p. 474
	// 2) Kruskal 알고리즘을 구현하였습니다. "graph.txt" 파일과 함께 작동하는 과정을 이해해 보세요.
	//WGraphMST g;
	//g.load("graph.txt");
	...
	// p. 490
	// 3) 다익스트라 알고리즘의 작동 과정을 이해하세요. "graph_sp.txt" 파일을 이용합니다. 
	//WGraphDijkstra g;
	...
	// p. 495
	// 4) 다익스트라 알고리즘은 한 정점을 시작 정점으로 지정하여 알고리즘이 작동합니다. 모든 정점에 대해 다익스트라 알고리즘을 실행하는 방법이 Floyd 알고리즘입니다. "graph_sp.txt" 파일을 이용합니다.
	WGraphFloyd f;
	string fn = "graph_sp.txt";
	f.load(fn);
