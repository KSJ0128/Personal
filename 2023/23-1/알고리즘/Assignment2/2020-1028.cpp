...
const int POPULATION_SIZE = 10; // 개체 집단 크기
...
Individual crossover(const Individual& parent1, const Individual& parent2) {
    ...
    int crossoverPoint = ver(gen); // 0 ~ 정점 개수 - 1, 어느 지점까지 crossover 적용할지
    ...
    initializePopulation(population);
        ...
        while (true) { // crossover&mutation 거친 newPopulation으로 업데이트 후 최적해 초기화 반복
            ...
            for (int i = 0; i < POPULATION_SIZE; i++) {
                ...
                {
                    if (hund(gen) < 10) { // 초기엔 10% 확률로 돌연변이 발생
                        mutate(child);
                    }
                }
                else
                {
                    if (hund(gen) < 1) { // 이후엔 1% 확률로 돌연변이 발생
                        mutate(child);
                    }
                }
